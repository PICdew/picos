...
crc24 base64_crc_octets(struct base64_stream *encoder,unsigned char *octets, size_t len)
{
   ...
   if(encoder == NULL)
      reason_error("base64_crc_octets: NULL pointer for encoding stream object\n"); 
...
void base64_encode_buffer(struct base64_stream *encoder)
{
    if(encoder == NULL || encoder->outblock == NULL)
        return;

   outblock[0] = (encoder->buffer[0] & 0xfc) >> 2;
   ...
   outblock[2] = ((buffer[1] & 0xf) << 4) | ((buffer[2] & 0xc0) >> 6);
...
void base64_encode(struct base64_stream *encoder, void *data, size_t size)
{
    ...
    for(;size > 0;size-=3)
    {
        full_assert(bytes_in_buffer < 3,"Internal Error: bytes_in_buffer should never exceed 3. It equals %d\n",encoder->bytes_in_buffer);
...
int main(int argc, char **argv)
{
    ...
    { 
        base64_encode(stdout,argv[counter],strlen(argv[counter]));
        fprintf(stdout,"\n=%ld",crc_octets((unsigned char*)argv[counter],strlen(argv[counter])));
        fprintf(stdout,"\n");
        fflush(stdout);
    }
