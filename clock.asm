...
INIT movlw 0x60;last memoryspot
	...
	movwf TRISC; Hours/~Minutes(7bit), Binary/~7-seg(6bit), When 7-seg is on bit 4 is ~minutes and bit 7 is ~hour
	movlw 0x6;
	...
	movwf TRISB;bit 7 = trigger for dip switch commands.
	...
	movlw 0x5F
	movwf datePointer
	movlw 0x10;by default light 2nd decimal (from left)
	movwf indicator
	movlw .29
	movwf dateDay;hehe
	movlw 0x6
...
MAIN_LOOP movf minutes,W
	...
	bcf myStatus,0
	bcf myStatus,1
	movlw 0x10;light appropriate indicator
	btfsc myStatus,3
	movlw 0x18;left most decimalpoint
	movwf indicator
	call CREATE_DISPLAY
	movlw 0xff
	movwf counter
DISPLAY_LOOP call DISPLAY_ME
	...
	;
CREATE_DISPLAY bcf myStatus,0 ;; display minutes
	bsf myStatus,1;binary first
	bsf myStatus,2;right first
...
MAKE_PACKET movwf tmp
	bcf tmp,7
	bcf tmp,6
	btfsc myStatus,0;hours/~min
	bsf tmp,7
	btfsc myStatus,1;~7-seg/binary
	bsf tmp,6
	btfsc myStatus,1
...
REST_OF_MAKE_PACKET movlw 0x0
	btfss myStatus,2;add offset for left 7-seg
	movlw 0x8
	addwf tmp,F
	movf tmp,W
	return
...
INC_MONTH movlw .1
	...
	movwf dateMonth
	;
PARSE_COMMAND movf dipControl,W
	movwf lastCommand
	bcf lastCommand,7
	movlw 0x0
	subwf lastCommand,W
